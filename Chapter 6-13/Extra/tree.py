...
class Tree(object):
    def __init__(self, root=None):
        self.root = root
        if type(root) != TreeNode and root is not None:
            raise TypeError
        if root is not None:
    ...
    def is_empty(self):
        return self.root.payload is None and len(self.root.children) == 0
    ...
    def pre_order_traverse(self, func=pwnl):
        ...
        if func == pwnl:
            print("")  # to force a newline after everything's printed
